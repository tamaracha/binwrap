...
module.exports = function prepare (config) {
  ...
  const binstubTemplate = fs.readFileSync(path.join(__dirname, 'binstub.js.mustache')).toString()

  config.binaries.forEach(function (bin) {
    const binPath = path.join('bin', bin)
    const content = binstubTemplate.replace(/{{ binName }}/g, JSON.stringify(bin))
